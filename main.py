...
def load_state():
	...
	# Return empty dict if state file doesn't exist
	if not os.path.exists(STATE_FILE):
		return {}
	try:
		# Load and parse JSON state file
...
def save_state(state):
	...
	try:
		# Write state to JSON file with proper encoding and formatting
		...
		async with aiohttp.ClientSession() as session:
			async with session.get(
				...
				if response.status == 401:
					is_refreshed, new_token_data = await check_and_refresh_on_401(
						response, None
					)
...
class Player(QMainWindow):
	...
	def __init__(self):
		...
		self.video.setFocus()
		self.video.setAttribute(Qt.WA_NoMousePropagation, False)
		...
		self.anime_label.setTextFormat(Qt.PlainText)
		...
		self.shikimori_link.setTextFormat(Qt.RichText)
	...
	def eventFilter(self, obj, event):
		if obj == self.video and event.type() == QEvent.MouseButtonDblClick:
			self.toggle_fullscreen()
			return True
		if obj == self.video and event.type() == QEvent.Resize and self.player:
	...
	def keyPressEvent(self, event):
		...
		if event.key() == Qt.Key_Space:
			...
			event.accept()
			return
		if event.key() == Qt.Key_F11 or event.key() in (Qt.Key_F, 1040):
			self.toggle_fullscreen()
			event.accept()
			return
		...
		if mod == Qt.ControlModifier and event.key() == Qt.Key_O:
			...
			event.accept()
			return
		if mod == Qt.ControlModifier and event.key() == Qt.Key_Q:
			self.close()
			event.accept()
			return
		if mod == Qt.ControlModifier and event.key() == Qt.Key_P:
			self.prev()
			event.accept()
			return
		if mod == Qt.ControlModifier and event.key() == Qt.Key_N:
			self.next()
			event.accept()
			return
		if mod == Qt.ControlModifier and event.key() == Qt.Key_S:
			self.stop_playback()
			event.accept()
			return
		if event.key() in (Qt.Key_Comma, Qt.Key_Less, Qt.Key_Left):
			self._seek_relative(-5)
			event.accept()
			return
		if event.key() in (Qt.Key_Period, Qt.Key_Greater, Qt.Key_Right):
			self._seek_relative(5)
			event.accept()
			return
	...
	def next(self):
		...
		# Делаем проверку, если файл просмотрен менее чем на 90%, не сохраняем прогресс и не отправляем на Шики, а просто запускаем следующий эпизод. Если же просмотрен более чем на 90%, сохраняем прогресс, отправляем на Шики и запускаем следующий эпизод.
		t = self.position 
		l = self.duration
		...
		if self.index >= len(self.playlist) - 1:
			if(status_update_rate): self.update_user_rate_if_needed()
			return
...
async def main():
	...
	logging.info(f"OAuth callback server started on port 3000")
